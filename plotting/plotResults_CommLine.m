...
f1=figure; imagesc( (negDiff) ); 
axis equal tight; caxis([-r2 -r1]); colorbar; colormap(gca, turbo(256)); title('Negative current B_{111} (gauss)'); set(gca,'YDir','normal');
saveas(f1, fullfile(myDir, folderName,  'negCurrent.png'),'png');

f2=figure; imagesc( (posDiff) ); axis equal tight; caxis([r1 r2]); colorbar; colormap(gca, turbo(256)); title('Positive current B_{111} (gauss)'); set(gca,'YDir','normal');
saveas(f2, fullfile(myDir, folderName,  'posCurrent.png'),'png');

f3=figure; imagesc( B111ferro ); axis equal tight; caxis(-.1 + [-rng rng]); colorbar; colormap(gca, turbo(256)); title('Positive + negative ferro B_{111} (gauss)'); set(gca,'YDir','normal');
saveas(f3, fullfile(myDir, folderName,  'ferromagImg.png'),'png');

f4=figure; imagesc(ledImg); axis equal tight; colorbar; colormap(gca, gray(256)); caxis auto; title('LED image'); set(gca,'YDir','normal');
saveas(f4, fullfile(myDir, folderName,  'ledImg.png'),'png');

f5=figure; imagesc( B111para ); axis equal tight; caxis([r1 r2]); colorbar; colormap(gca, turbo(256)); title('Positive + negative para B_{111} (gauss)'); set(gca,'YDir','normal');
...
s1 = subplot(2,2,1); imagesc( (negDiff) ,'hittest', 'off'); axis equal tight; caxis auto; colorbar; colormap(s1, turbo(256)); title('Negative current B_{111} (gauss)'); set(gca,'YDir','normal');
s2 = subplot(2,2,2); imagesc( (posDiff) ,'hittest', 'off'); axis equal tight; caxis auto; colorbar; colormap(s2,turbo(256)); title('Positive current B_{111} (gauss)'); set(gca,'YDir','normal');
s3 = subplot(2,2,3); imagesc( B111ferro ,'hittest', 'off'); axis equal tight;  caxis(mean2(B111ferro) + [-rng rng]); colorbar; colormap(s3,turbo(256)); title('Positive + negative ferro B_{111} (gauss)'); set(gca,'YDir','normal');
