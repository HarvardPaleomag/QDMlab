function results2MaGIC(data_files,results_fp,dest,contrib,experiment,specimen,kwargs,magic)
% This function takes a results file (output of fit_source_series) and,
% optionally, a set of file paths contained in that fiels and performs
% three (four) main tasks for producing a MagIC dataset:
%   1) Writes information into a supplied MagIC contribution file
...
% results_fp: str
%   path to results file (struct: output of dipole_fit_series) 
% dest: str
%   path to save MagIC files
% contrib: str
%   file path of MagIC contribution file (should already contain locations,
%   sites, samples and specimens tables)
% experiment: str
%   experiment name
% specimen: str
...
% writeToContrib: bool [true]
%   whether to write data into contribution file
...
%% prepare data
% load results file
results = load(results_fp,'results').results;
...
% check whether we'll need to write column names
writeColNames = 0;
if kwargs.writeToContrib
    % open MagIC contribution file
    fileIDContrib = fopen(contrib,'a+');
    
    frewind(fileIDContrib)
    temp = textscan(fileIDContrib,'%s','Delimiter','\n');
    if isempty(temp{1})
        writeColNames = 1;
...
if writeColNames
    ...
    fprintf(fileIDContrib,colNames);
end
...
if ~kwargs.nameFunction
    ...
    for s = 1:length(data_files)
        names(s) = {[experiment, '_',num2str(s)]};
    end
else
...
for j = 1:length(data_files)
    ...
    B111FName = [dest,'magic_measurements_',num2str(meas_n+j),'.txt'];
    BzFName = [dest,'magic_measurements_',num2str(meas_n+j),'.txt'];
    ...
    if kwargs.makeImgs
        ...
        if kwargs.saveBzFitted
            ...
            BzTemp = UpCont(BzTemp, results.UC{1}, 1/kwargs.nPixels);
            ...
            % write fitted map FP to file
        end
    end

%% make MagIC contribution file
    if kwargs.writeToContrib
        % make files names and image strings cor contribution file
        ...
        % get transformed FOV
        try
            [xCorr,yCorr] = transformPointsInverse(results.tforms{i},(results.xMax(i) + results.xMin(i))/2,...
                (results.yMax(i) + results.yMin(i))/2);
        catch
           [xCorr,yCorr] = transformPointsInverse(tf.nTransForms{file},(results.xMax(i) + results.xMin(i))/2,...
                (results.yMax(i) + results.yMin(i))/2);
        end
        ...
        % print line to file
        fprintf(fileIDContrib, contribLine);
    end
end

% cleanup
fclose(fileIDContrib);
close all
