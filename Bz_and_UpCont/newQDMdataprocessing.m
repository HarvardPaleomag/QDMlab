...
if strcmp(ferroorpara,'ferro')
    if beta
        B=imrotate(B111ferro,beta,'bilinear') * 1e-6; % Field in T
    else
        B=imrotate(B111ferro,0) * 1e-6; % Field in T
    end
else
    if strcmp(ferroorpara,'para')
        if beta
            B=imrotate(B111para,beta,'bilinear') * 1e-6 ; % Field in T
        else
            B=imrotate(B111para,0) * 1e-6; % Field in T
        end
    else
